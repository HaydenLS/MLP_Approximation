...
class MLP:
    ...
    def train(self, X, y, lr, epochs, activate_function=AF.tanh, activate_derivative=AF.tanh_derivative):
        self.loss_history = []  # Для хранения потерь
        for epoch in range(epochs):
            ...
            self.loss_history.append(loss)  # Сохранение потери
    ...
    # Метод SDG
    def train_SDG(self, X, y, lr, epochs, batch_size, activate_function=AF.tanh,
...
# Графики потерь
def plot_loss(gd_loss, sgd_loss, sdg_mini_loss):
    plt.plot(gd_loss, label="GD Loss")
    plt.plot(sgd_loss, label="SGD Loss")
    plt.plot(sdg_mini_loss, label="SGD Mini Loss")
    plt.xlabel("Epochs")
    plt.ylabel("Loss")
    plt.legend()
    plt.title("GD vs SGD vs MiniSDG Loss")
    plt.show()
...
# Activate Functions
activate_function = AF.leaky_relu
activate_derivative = AF.leaky_relu_derivative


# Градиентный спуск
model_gd = DoubleLayerMLP(input_dim, l1_dim, l2_dim, output_dim)
model_gd.train(X, y_true, 0.001, 5000, activate_function, activate_derivative)

# Стохастический градиентный спуск
model_sgd = DoubleLayerMLP(input_dim, l1_dim, l2_dim, output_dim)
model_sgd.train_SDG(X, y_true, 0.01, 5000, 1, activate_function, activate_derivative)

# Стохастический градиентный спуск с размером батчей
model_sgd_mini = DoubleLayerMLP(input_dim, l1_dim, l2_dim, output_dim)
model_sgd_mini.train_SDG(X, y_true, 0.01, 5000, 20, activate_function, activate_derivative)


# График функции после обучения
plt.title("Model_GD")
plt.plot(X, y_true, label="Actual function")
plt.plot(X, model_gd.forward(X, activate_function), label="MLP approximation")
plt.legend()
plt.show()
...
plt.title("Model_SGD")
plt.plot(X, y_true, label="Actual function")
plt.plot(X, model_sgd.forward(X, activate_function), label="MLP approximation")
plt.legend()
plt.show()

plt.close()

plt.title("Model_SGD_Mini")
plt.plot(X, y_true, label="Actual function")
plt.plot(X, model_sgd_mini.forward(X, activate_function), label="MLP approximation")
plt.legend()
plt.show()



plot_loss(model_gd.loss_history, model_sgd.loss_history, model_sgd_mini.loss_history)